...
def main():
    ...
    for file in behav_files:
        ...
        fMRI_image_indexes = [(int((x - 2) / 2)-1, int(x / 2)-1) if x % 2 == 0 else (int((x - 1) / 2), int((x + 1) / 2)) for
                              x in stimulus_onsets_int]

        for index, fMRI_image_index in enumerate(fMRI_image_indexes):
            # print(index, fMRI_image_index)
            output_df.iloc[index, 0] = abs(fft(eeg['data_noGA'][:43, stimulus_onsets[index]:stimulus_onsets[index] + 1000])[:,1:151])
